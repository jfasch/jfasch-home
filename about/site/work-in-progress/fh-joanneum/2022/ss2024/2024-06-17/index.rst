.. include:: <mmlalias.txt>


2024-06-17(6): C++: Container Templates
=======================================

.. contents::
   :local:

``std::vector<>``, ``std::map<>``
---------------------------------

From :doc:`/trainings/material/soup/cxx03/060-stl/group`

* :doc:`/trainings/material/soup/cxx03/060-stl/010-basics/topic`
  (`ptr-arith-stl.cpp
  <https://github.com/jfasch/FH-STECE2022/blob/main/livecoding/ptr-arith-stl.cpp>`__)
* :doc:`/trainings/material/soup/cxx03/060-stl/015-vector-copy-etc/topic`
* :doc:`/trainings/material/soup/cxx03/060-stl/040-sequential-containers/vector`
  (`vector-demo.cpp
  <https://github.com/jfasch/FH-STECE2022/blob/main/livecoding/vector-demo.cpp>`__,
  `vector-demo2.cpp
  <https://github.com/jfasch/FH-STECE2022/blob/main/livecoding/vector-demo2.cpp>`__)
* :doc:`/trainings/material/soup/cxx03/060-stl/050-associative-containers/map`
  (`map-demo.cpp
  <https://github.com/jfasch/FH-STECE2022/blob/main/livecoding/map-demo.cpp>`__)

``auto``
--------

From
:doc:`/trainings/material/soup/cxx11/020-new-language-features/group`

* :doc:`/trainings/material/soup/cxx11/020-new-language-features/auto`
