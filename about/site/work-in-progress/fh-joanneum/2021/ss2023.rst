...
.. list-table::
   ...
   * * 2023-06-02
     ...
     * 
       ...
       * *Solution (livehacked)*
         ...
         * CMake recap (`CMake Documentation here
           <https://cmake.org/cmake/help/latest/>`__)
  
	   * Source dir

	     * Sketch ``bin/``, ``lib/``, ``tests/``
	     * ``CMakeLists.txt``
        
	       * Structural: intermediate; ``ADD_DIRECTORY()``
	       * Structural: `entry point, toplevel
                 <https://github.com/jfasch/FH-ECE21/blob/main/CMakeLists.txt>`__)
        
		 * |longrightarrow| ``ADD_DIRECTORY()`` to decend into
		   subdirs
    
	   * Build: *build *dirs* |longrightarrow| ``cmake
	     $SOURCEDIR`` creates parallel structure in *current
	     working directory*.
	   
	     * Build |longrightarrow| ``make -j``
	     * Run test suite |longrightarrow| see it fail
	   ...
	   * Exercise
	     ...
	     * Setup initial status
	       ...
	       * ``ADD_EXECUTABLE()`` |longrightarrow| dependencies
		 (show dependencies of my own solution, monolithic)
	       * ``ADD_LIBRARY()``? ``TARGET_LINK_LIBRARIES()``?
	       * Download, and CMakery to build monolithic sensor
		 implementation |longrightarrow| ``bin/``!
	       * Build in topdir ... stop
	       * Build in subdir (faster), run from there, on a
                 simulation, on ``x86_64``
	   ...
	   * Exercise:
	     ...
	     * Initial status
	       ...
	       * Download, and CMakery to build monolithic sensor
		 implementation |longrightarrow| ``bin/``!
	       * Test only locally, with simu file |longrightarrow|
		 ARM later
	       * Add :download:`test suite
		 </trainings/material/soup/cxx-code/oo-sensor-w1/bin/onewire-temperature-monolithic.cpp>`
		 to test runner |longrightarrow| ``tests/``!

	     * Add test

	       * Download and add test
	       * CMakery: add ``lib`` 's name to test runner
	       * Enter TDD ...
   ...
   * * 2023-06-05
     ...
     * 
       ...
       * :doc:`/trainings/material/soup/cxx03/060-stl/015-vector-copy-etc/vector-basic`
