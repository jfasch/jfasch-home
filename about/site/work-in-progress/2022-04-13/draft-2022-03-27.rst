...
* |longrightarrow| ``UserDB db``, together with setup, defined inside
  ...
  * A second test, later, when a feature is implemented
    during TDD jjj link, will do the same.
...
* Restructure test code
  ...
  * Provide a global variable and a ``setup_fixture(std::vector)``
    (*by copy*, provocantly [#rvalue-ref]_) in
    ``userdb-fixture.{h.cpp}``)
