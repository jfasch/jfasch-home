...
{
    ssize_t nwritten = write(sig_channel_write, &signal, sizeof(signal));
    if (nwritten == -1)
        exit(2);                                       // <-- perror not async-signal-safe
}
...
int main()
{
    ...
    while (!quit) {
        int signal;
        ssize_t nread = read(sig_channel_read, &signal, sizeof(signal));
        if (nread == -1) {
