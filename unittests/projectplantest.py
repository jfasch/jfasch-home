...
class ProjectPlanTest(unittest.TestCase):
    def test_basic(self):
        ...
        task_a = Task(title='TaskA', path=('package', 'task_a'), docname='docname-task-a.rst', dependencies=[])
        task_b = Task(title='TaskB', path=('package', 'task_b'), docname='docname-task-b.rst', dependencies=['package', 'task_a'])
        
        package = Group(title='Workpackage', path=('package',), docname='docname-package.rst')
